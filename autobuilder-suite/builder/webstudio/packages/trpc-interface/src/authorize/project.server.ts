...
export const hasProjectPermit = async (
  ...
  console.log("[hasProjectPermit] Authorization type:", authorization.type);

  if (authorization.type === "anonymous") {
    console.log("[hasProjectPermit] Returning false - anonymous user");
    return false;
  }

  // Handle each authorization type explicitly
  if (authorization.type === "service") {
    console.log("[hasProjectPermit] Service context - checking permit:", props.permit);
    // Service context has full access for internal operations like static export
    // It can view, edit, and build - but not own or admin
    return props.permit === "view" || props.permit === "edit" || props.permit === "build";
  }

  if (authorization.type === "user") {
    console.log("[hasProjectPermit] Checking permit for user:", authorization.userId);
  ...
  if (authorization.type === "token") {
    console.log("[hasProjectPermit] Checking permit for token");
    return checkProjectPermit(
      props.projectId,
      props.permit,
      authorization,
  ...
  const _exhaustiveCheck: never = authorization;
  console.log("[hasProjectPermit] Unknown authorization type:", _exhaustiveCheck);
  return false;
};
