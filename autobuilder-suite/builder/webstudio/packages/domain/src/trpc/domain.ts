    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const result = await deploymentTrpc.publish.mutate({
          ...
          // For static exports: pass the filename so the handler uses the same name
          exportName: input.destination === "static" ? name : undefined,
        });

        if (input.destination === "static" && result.success) {
          // For static exports, use BUILDER_ORIGIN which is the current request origin
          // This ensures the download stays on the same domain (important for wstd.dev local dev)
          const downloadUrl = `${env.BUILDER_ORIGIN}/cgi/static/ssg/${name}`;
          console.log(`[Static Export] Download URL: ${downloadUrl}`);
          return { success: true as const, name, downloadUrl };
        }
