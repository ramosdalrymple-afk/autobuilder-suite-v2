          ...
          onClick={() => {
            startTransition(async () => {
              try {
                ...
                // Get the full download URL (includes builder origin for cross-origin support)
                const downloadUrl = "downloadUrl" in result ? result.downloadUrl : undefined;
                ...
                if (status === "PUBLISHED") {
                  // Try to download using the full URL, fall back to relative path
                  const url = downloadUrl ?? `/cgi/static/ssg/${name}`;
                  
                  // If downloadUrl is a different origin (e.g., http://localhost from https://wstd.dev)
                  // we need to use fetch with CORS or open in a new window
                  if (downloadUrl && new URL(downloadUrl).origin !== window.location.origin) {
                    // Open in new window/tab to avoid mixed content issues
                    window.open(downloadUrl, "_blank");
                    toast.success("Download opened in new tab. If blocked, please allow popups.");
                  } else {
