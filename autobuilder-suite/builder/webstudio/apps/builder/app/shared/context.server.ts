...
export const extractAuthFromRequest = async (request: Request) => {
  ...
  // In production, require matching TRPC_SERVER_API_TOKEN
  // In local dev (when no token is configured), accept "local-dev-token"
  const authHeader = request.headers.get("Authorization");
  const isServiceCall =
    authHeader !== null &&
    (authHeader === env.TRPC_SERVER_API_TOKEN ||
      (env.TRPC_SERVER_API_TOKEN === undefined && authHeader === "local-dev-token"));
...
const createAuthorizationContext = async (
  ...
  console.log("[createAuthorizationContext] sessionData:", sessionData);
  console.log("[createAuthorizationContext] authToken:", authToken ? "present" : "none");
  console.log("[createAuthorizationContext] isServiceCall:", isServiceCall);
  ...
  if (sessionData?.userId != null) {
    ...
    console.log("[createAuthorizationContext] Returning 'user' type with userId:", userId);
  ...
  console.log("[createAuthorizationContext] Returning 'anonymous' type - no session data");
...
const createDeploymentContext = (builderOrigin: string) => {
  const context: AppContext["deployment"] = {
    ...
    env: {
      ...
      LOCAL_BUILDER_URL: env.LOCAL_BUILDER_URL,
    },
  };
